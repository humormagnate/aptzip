...
public class BoardController {
	...
	@PostMapping("/write")
	public String writePost(BoardEntity board, @AuthenticationPrincipal UserResponseDto principal, RedirectAttributes rttr, String categoryId)
		throws Exception {
		
		log.debug("/board/write/post >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
		log.debug(board.toString());
		log.debug(principal.toString());
		
		if (("".equals(board.getBoardTitle())) || ("".equals(categoryId) && categoryId == null)) return "";
		...
		log.debug(board.toString());
		log.debug(board.getUser().getApt().toString());
	...
	@ResponseBody
	@PutMapping("/{id}")
	public String put(@PathVariable("id") Long id, @ModelAttribute BoardEntity board, String categoryId) {
		log.debug("====================================Put Mapping=================================");
		if ("".equals(categoryId) && categoryId == null) return "";
		log.debug("boardId : {}", id.toString());
		log.debug("board : {}", board.toString());
		log.debug("categoryId : {}", categoryId);
		
		boardRepo.updateById(
			id,
			board.getBoardTitle(),
